...
export function SearchInput(){
    ...
    return(
        <div className="flex bg-gray rounded-lg justify-between items-center text-gray px-4 py-2 mx-4 mb-5 flex-row-reverse relative">
            ...
            <span onClick={()=>!isSearchLoading ? clearSearchInput() : {}} className={`opacity-0 peer peer-focus:opacity-100 flex absolute top-1/2 -translate-y-1/2 right-4 rounded-full ${!isSearchLoading && 'bg-zinc-300'} size-4 justify-center items-center cursor-pointer`}>
                {isSearchLoading ?
    ...
    return(
        <Link onClick={handleSearchClick} href={type == 'tag' ? `/tags/${id}` : `/${id}`} className="py-2 px-6 flex gap-3 items-center cursor-pointer bg-white hover:bg-gray">
            ...
            <div className="flex flex-1 flex-col text-sm leading-[18px] relative">
                ...
                {type == 'tag' ? 
                    localHistory ?
                        <span className="text-gray truncate">{subtitle} posts</span>
                        :
                        (
                            subtitle < 100 ?
                            <span className="text-gray truncate">Fewer than 100 posts</span>
                            :
